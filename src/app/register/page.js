...
function Register() {
  ...
  const submitHandler = async (event) => {
    ...
    try {
      if (newUser.name?.trim().length < 4 || newUser.phoneNumber?.trim().length < 11 || newUser.email?.trim().length < 10 || newUser.password?.trim().length < 8) {
  ...
  return (
    <>
      <div className="mt-12 sm:mt-5 cnplf min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-gray-900 via-blue-900 to-gray-900 !flex items-center justify-center !p-6 !overflow-x-hidden !overflow-y-hidden">
