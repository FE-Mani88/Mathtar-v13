...
export default function AllTemplate({id,  questions, image, title, difficulty, topics, grade, totalQuestions, description, duration }) {

    const [isQuizStarted, setIsQuizStarted] = useState(null)

    if (isQuizStarted) {
        return <ActiveQuiz questions={questions} duration={duration} title={title} id={id} />;
    }
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8 text-center !flex justify-evenly">
                                ...
                                <div className="flex items-center gap-2 space-x-3 text-gray-700">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-medium dark:text-white flex gap-1"> <span>سوال</span>{totalQuestions} </p>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center space-x-3 text-gray-700 gap-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-medium dark:text-white">{grade === 7 ? "هفتم" : grade === 8 ? "هشتم" : "نهم"}</p>
                                    </div>
                                </div>
